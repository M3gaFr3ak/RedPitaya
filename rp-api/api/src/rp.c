...
int rp_Init() { return rp_InitReset(true); }
...
int rp_IsApiInit() { return g_api_state; }
...
rp_calib_params_t rp_GetCalibrationSettings() { return calib_GetParams(); }

int rp_CalibrateFrontEndOffset(rp_channel_t channel, rp_pinState_t gain,
                               rp_calib_params_t *out_params) {
...
int rp_CalibrationReset() { return calib_Reset(); }

int rp_CalibrationFactoryReset() { return calib_LoadFromFactoryZone(); }

int rp_CalibrationSetCachedParams() { return calib_setCachedParams(); }
...
int rp_DpinSetDirection(rp_dpin_t pin, rp_pinDirection_t direction) {
  ...
  if (pin < RP_DIO0_P) {
    ...
    iowrite32((tmp & ~(1 << pin)) | ((direction << pin) & (1 << pin)),
              &hk->ex_cd_p);
  } else {
    // DIO_N
    pin -= RP_DIO0_N;
...
int rp_DpinSetState(rp_dpin_t pin, rp_pinState_t state) {
  ...
  if (pin < RP_DIO0_P) {
    ...
    iowrite32((tmp & ~(1 << pin)) | ((state << pin) & (1 << pin)),
              &hk->led_control);
  } else if (pin < RP_DIO0_N) {
    // DIO_P
    pin -= RP_DIO0_P;
...
int rp_ApinReset() { return rp_AOpinReset(); }
...
int rp_AIpinGetValueRaw(int unsigned pin, uint32_t *value) {
  ...
  switch (pin) {
  case 0:
    fp = fopen("/sys/devices/soc0/amba_pl/83c00000.xadc_wiz/iio:device1/"
               "in_voltage11_vaux8_raw",
               "r");
    break;
  case 1:
    fp = fopen("/sys/devices/soc0/amba_pl/83c00000.xadc_wiz/iio:device1/"
...
int rp_AIpinGetValue(int unsigned pin, float *value) {
  ...
  *value = (((float)value_raw / ANALOG_IN_MAX_VAL_INTEGER) *
...
int rp_AOpinGetValue(int unsigned pin, float *value) {
  ...
  *value = (((float)value_raw / ANALOG_OUT_MAX_VAL_INTEGER) *
            (ANALOG_OUT_MAX_VAL - ANALOG_OUT_MIN_VAL)) +
           ANALOG_OUT_MIN_VAL;
  return result;
}
...
int rp_AcqSetArmKeep(bool enable) { return acq_SetArmKeep(enable); }

int rp_AcqGetArmKeep(bool *state) { return acq_GetArmKeep(state); }
...
int rp_AcqConvertFactorToDecimation(uint32_t factor,
...
int rp_AcqSetAveraging(bool enabled) { return acq_SetAveraging(enabled); }

int rp_AcqGetAveraging(bool *enabled) { return acq_GetAveraging(enabled); }
...
int rp_AcqGetTriggerHyst(float *voltage) { return acq_GetTriggerHyst(voltage); }

int rp_AcqSetTriggerHyst(float voltage) { return acq_SetTriggerHyst(voltage); }

int rp_AcqGetWritePointer(uint32_t *pos) { return acq_GetWritePointer(pos); }

int rp_AcqGetWritePointerAtTrig(uint32_t *pos) {
  return acq_GetWritePointerAtTrig(pos);
}

int rp_AcqStart() { return acq_Start(); }

int rp_AcqStop() { return acq_Stop(); }
int rp_AcqReset() { return acq_Reset(); }

int rp_AcqResetFpga() { return acq_ResetFpga(); }

uint32_t rp_AcqGetNormalizedDataPos(uint32_t pos) {
  return acq_GetNormalizedDataPos(pos);
}

int rp_AcqGetDataPosRaw(rp_channel_t channel, uint32_t start_pos,
                        uint32_t end_pos, int16_t *buffer,
                        uint32_t *buffer_size) {
  return acq_GetDataPosRaw(channel, start_pos, end_pos, buffer, buffer_size);
}

int rp_AcqGetDataPosV(rp_channel_t channel, uint32_t start_pos,
                      uint32_t end_pos, float *buffer, uint32_t *buffer_size) {
  return acq_GetDataPosV(channel, start_pos, end_pos, buffer, buffer_size);
}

int rp_AcqGetDataRaw(rp_channel_t channel, uint32_t pos, uint32_t *size,
                     int16_t *buffer) {
  return acq_GetDataRaw(channel, pos, size, buffer);
}

int rp_AcqGetDataRawV2(uint32_t pos, uint32_t *size, uint16_t *buffer,
                       uint16_t *buffer2) {
  return acq_GetDataRawV2(pos, size, buffer, buffer2);
}

int rp_AcqGetOldestDataRaw(rp_channel_t channel, uint32_t *size,
                           int16_t *buffer) {
  return acq_GetOldestDataRaw(channel, size, buffer);
}

int rp_AcqGetLatestDataRaw(rp_channel_t channel, uint32_t *size,
                           int16_t *buffer) {
  return acq_GetLatestDataRaw(channel, size, buffer);
}

int rp_AcqGetDataV(rp_channel_t channel, uint32_t pos, uint32_t *size,
                   float *buffer) {
  return acq_GetDataV(channel, pos, size, buffer);
}

int rp_AcqGetDataV2(uint32_t pos, uint32_t *size, float *buffer1,
                    float *buffer2) {
  return acq_GetDataV2(pos, size, buffer1, buffer2);
}

int rp_AcqGetDataV2D(uint32_t pos, uint32_t *size, double *buffer1,
                     double *buffer2) {
...
int rp_AcqGetBufSize(uint32_t *size) { return acq_GetBufferSize(size); }
...
int rp_AcqGetFilterCalibValue(rp_channel_t channel, uint32_t *coef_aa,
                              uint32_t *coef_bb, uint32_t *coef_kk,
                              uint32_t *coef_pp) {
...
int rp_GenReset() { return gen_SetDefaultValues(); }

int rp_GenOutDisable(rp_channel_t channel) { return gen_Disable(channel); }

int rp_GenOutEnable(rp_channel_t channel) { return gen_Enable(channel); }
...
int rp_GenGetArbWaveform(rp_channel_t channel, float *waveform,
                         uint32_t *length) {
...
int rp_GenTrigger(uint32_t channel) { return gen_Trigger(channel); }

int rp_GenSynchronise() { return gen_Synchronise(); }

int rp_GenResetTrigger(rp_channel_t channel) {
  return gen_ResetTrigger(channel);
}

int rp_GenOutEnableSync(bool enable) { return gen_EnableSync(enable); }

float rp_CmnCnvCntToV(uint32_t field_len, uint32_t cnts, float adc_max_v,
                      uint32_t calibScale, int calib_dc_off,
                      float user_dc_off) {
...
int rp_UartInit() { return uart_Init(); }

int rp_UartRelease() { return uart_Release(); }
...
int rp_UartSpeed(int value) { return uart_SetSpeed(value); }

int rp_UartSetBits(rp_uart_bits_size_t _size) { return uart_SetBits(_size); }
...
int rp_GetLEDMMCState(bool *_enable) { return led_GetMMCState(_enable); }

int rp_SetLEDMMCState(bool _enable) { return led_SetMMCState(_enable); }
...
int rp_GetLEDEthState(bool *_state) { return led_GetEthState(_state); }

int rp_SetLEDEthState(bool _state) { return led_SetEthState(_state); }
